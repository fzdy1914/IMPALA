...
from agent.boiler_goose import FloodGoose
...
def actor(idx, q, data, env, is_training_done, args):
    ...
    with torch.no_grad():
        ...
        trajectory_list = [Trajectory(), Trajectory()]

        while not is_training_done.is_set():
            risk_agent = RiskAverseGreedyAgent()
            flood_agent = FloodGoose()
            ...
            while not env.done():
                ...
                row_obs = state[0]["observation"]

                if state[2]['status'] == 'ACTIVE':
                    obs_risk = deepcopy(row_obs)
                    obs_risk["index"] = 2
                    actions.append(risk_agent.agent(obs_risk, config))
                else:
                    actions.append("NORTH")
                if state[3]['status'] == 'ACTIVE':
                    obs_flood = deepcopy(row_obs)
                    obs_flood["index"] = 3
                    actions.append(flood_agent(obs_flood, config))
                else:
                    actions.append("NORTH")
